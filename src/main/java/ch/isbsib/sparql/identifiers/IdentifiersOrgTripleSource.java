...
public class IdentifiersOrgTripleSource implements TripleSource {
	...
	@Override
	public CloseableIteration<Statement, QueryEvaluationException> getStatements(Resource subj, IRI pred, Value obj,
	...
	private Iterator<Statement> getIterViaDao(Resource subj, Value obj, Resource... contexts) {
	...
	private Iterator<Statement> getResultsViaDao(final Resource subj, Resource... contexts) {
	...
	private Iterator<Statement> getResultsViaDao(final Resource subj, final Resource obj, Resource... contexts) {

		Boolean activeflag = getActiveFlag(contexts);
		if (subj == null) {
			final Iterator<URIextended> iter = dao.getSameAsURIs(obj.stringValue(), activeflag).iterator();
