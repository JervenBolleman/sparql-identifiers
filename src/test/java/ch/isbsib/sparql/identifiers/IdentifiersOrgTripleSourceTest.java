...
public class IdentifiersOrgTripleSourceTest {
	...
	private final class RegistryDaoMock implements Dao {
		...
		public List<URIextended> getSameAsURIs(String uri, Boolean activeFlag) {
			...
			if (uri.equals("http://www.ebi.ac.uk/QuickGO/GTerm?id=GO:0006915")
					|| uri.equals("https://www.ebi.ac.uk/QuickGO/GTerm?id=GO:0006915")) {
				urls.add(new URIextended("http://www.ebi.ac.uk/QuickGO/GTerm?id=GO:0006915", true));
				urls.add(new URIextended("https://www.ebi.ac.uk/QuickGO/GTerm?id=GO:0006915", false));
	...
	String query1https = "PREFIX " + OWL.PREFIX + ": <" + OWL.NAMESPACE
			+ ">\n SELECT ?target WHERE {<https://www.ebi.ac.uk/QuickGO/GTerm?id=GO:0006915> owl:sameAs ?target}";
	...
	@Test
	public void testBasicHttpsMatch()
			throws IOException, QueryEvaluationException, MalformedQueryException, RepositoryException, SailException {
		...
		SailRepository sr = new SailRepository(rep);
		rep.init();
		TupleQuery pTQ = sr.getConnection().prepareTupleQuery(QueryLanguage.SPARQL, query1https);
		TupleQueryResult eval = pTQ.evaluate();
		for (int i = 0; i < GO_RESOURCES; i++) {
			// for (int i = 0; i < 10; i++) {
			assertTrue(eval.hasNext());
	...
	public void testGo()
		...
		while (eval.hasNext()) {
			final BindingSet next = eval.next();
			assertNotNull(next);
			assertTrue(next.getBinding("target").getValue().toString().contains("GO"));
		}
	}
}
